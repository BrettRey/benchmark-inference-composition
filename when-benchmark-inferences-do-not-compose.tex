...
First, projectibility is a property of a claim rather than of an object. Its bearer is one bounded extension: this result, to these unobserved cases, under these conditions, and not a score, benchmark, or system considered in isolation. The same benchmark result may provide substantial warrant for performance on further items generated under a registered template and little warrant for performance on open-ended professional work. Calling the benchmark \enquote{projectible} without naming the target suppresses the point at issue.
...
The contribution is a specialization. Composition becomes a distinct problem because of how AI evidence is produced: from separate studies, run by different actors, assembled later as though favourable conclusions were automatically adjacent. What that situation needs is operational, a representation in which separately produced claims can be checked field by field for whether they form a well-typed path, and a division between links that never meet and links that meet while warrant fails to cross. Kane's framework makes room for the assumptions at those interfaces. It leaves their identification to the analyst.
...
Projectibility audit & Specified extension and its interfaces & What warrants this source--target extension, and can it be joined to adjacent extensions? & Supplies an explicit interface record and diagnostic classification. \\
\bottomrule
\end{tabular}
\end{table}
...
\node[audit] (align) {\textbf{Endpoint alignment}\\[2pt]
  ...
  \(\mathcal{N}_1\ne\mathcal{N}'_1\Rightarrow\) bridge owed; not well-typed};
...
Let \(B\) be a benchmark result, \(D\) a draft-level outcome, and \(Z\) a final outcome after review. The probabilities below concern a system--request pair drawn at random from a target population \(T\): \(B\) varies across systems, while \(D\) and \(Z\) vary across pairs. In \(T\),
...
The firm sets the confirmatory rule before opening the later block. For each request type and condition, support for the empirical performance premise of mandatory-review use requires both (i) a one-sided 95\% exact upper confidence bound below a post-review substantive-defect tolerance of 3\% and (ii) no unresolvable citation in a final memo. The claim is defeated if the corresponding lower bound exceeds 3\% or if a red-line citation remains. Other results are unresolved. The red-line rule is a decision constraint rather than an estimate: observing no unresolvable citation in 200 memos doesn't establish that the rate is zero, and the report should carry the estimated rate and its uncertainty alongside the registered rule. Four request-type and condition combinations are tested and authorization is sought only for those that pass, so the family of candidate authorizations is registered in advance and the criterion applied to each is stated as one of four rather than as a single test. The four claims are interpreted separately, and the procedure makes no family-level false-confirmation guarantee. A firm that needs one, because any passing condition will be selected for authorization, should register simultaneous bounds or a hierarchical analysis instead. The 3\% tolerance isn't supplied by the benchmark or proposed as a general legal standard. In the example, the firm would have to justify it by comparing the consequences and costs of the available policies.
...
State B isn't only a possibility. In one released design that adds task-irrelevant context to benchmark items \citep{zhang2026illusionRobustness}, the median absolute signed change across 32 model--benchmark comparisons was .0068, while the median item instability, the mean absolute item-level change, was .0678. That's under one percentage point of net movement alongside nearly seven points of two-sided item-level movement. Those comparisons share items within benchmark and lineage across models, so they're one crossed dataset rather than 32 independent replications, and what they show is how that dataset behaves rather than how evaluations behave in general.
...
The audit is also relative to its own representation. Whether a given failure counts as endpoint mismatch or as failure of warrant transmission depends on the schema the evaluator declared, and a richer schema can convert the second into the first. That doesn't make the distinction arbitrary, since the two are found and repaired differently, but it does mean the classification records how finely the claim was described rather than a fact about the world independent of the description.
...
Validity-centred AI evaluation has made an essential correction: validity belongs to a proposed interpretation and use, not to a benchmark in isolation. This paper takes up what that correction leaves to the analyst, the joins between separately produced claims, and gives them a representation and an audit. A benchmark result may support several individually warranted steps without warranting the chain assembled from them. A capability attribution may support one step, but it doesn't itself specify the downstream cases or criterion relation.

Projectibility concerns whether one bounded extension is warranted. Goodman shows why source fit alone can't choose among rival extensions, and argument-based validity locates each extension among explicit claims and assumptions. A projectibility audit first asks whether links presented as adjacent meet in object, population, conditions, outcome, and period. A mismatch leaves the proposed composition ill-typed unless separately bridged. If the fields align, warrant still fails to transmit when assumptions or effect modifiers conflict or when dependence and uncertainty are lost.
