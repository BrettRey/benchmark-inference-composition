...
The contribution is a specialization, and composition becomes a distinct problem because of how AI evidence is actually produced. It arrives from separate studies, run by different actors, assembled later as though favourable conclusions were automatically adjacent. What that situation needs is an operational one: a representation in which separately produced claims can be checked field by field for whether they form a well-typed path, and a division between links that never meet and links that meet while warrant fails to cross. Kane's framework makes room for the assumptions at those interfaces. It leaves their identification to the analyst.
...
Projectibility audit & Specified extension and its interfaces & What warrants this source--target extension, and can it be joined to adjacent extensions? & Made the object of analysis rather than left to the analyst. \\
\bottomrule
\end{tabular}
\end{table}
...
State B isn't only a possibility. In one released design that adds task-irrelevant context to benchmark items \citep{zhang2026illusionRobustness}, the absolute signed change across 32 model--benchmark comparisons had a median of .0068 while two-sided item movement had a median of .0678. The typical mean moved by under a percentage point where the typical item set moved by nearly seven. Those comparisons share items within benchmark and lineage across models, so they're one crossed dataset rather than 32 independent replications, and what they show is how that dataset behaves rather than how evaluations behave in general.
