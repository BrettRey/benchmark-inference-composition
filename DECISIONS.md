...
2026-08-09 — The eleven cited-author outreach emails drafted on 2026-07-29 were never sent. Brett confirmed this today. Recorded because nothing said so: the draft file opens "These drafts are ready to send while the arXiv submission is processing," `DECISIONS.md` was silent, and `correspondence/` holds only the unrelated Pax Machina pitch. Reading the drafts, a session concluded the eleven were plausibly connected to the work and therefore ineligible as suggested reviewers under Springer's "totally independent and not connected to the work in any way" rule, which is the wrong conclusion and removed the strongest candidates from consideration. Freiesleben, Reuel, Salaudeen, Bean, Zhang, Binette, Raji, Bowman, Koyejo, Ho and Magesh are all eligible.

**If they are ever sent, that changes.** Soliciting a cited author's attention about this paper makes them connected to it, and anyone emailed must then be dropped from the suggested-reviewer list or disclosed. Update this entry on the day any of them goes out.

2026-08-09 — Suggested reviewers for the Minds and Machines submission, six, none contacted. Freiesleben (arXiv:2603.15121) is the nearest rival, answered in §2.3 and §6.2, and is the best-qualified reader as well as the likeliest to reject; suggested deliberately, on the view that the paper's answer to him either survives him or will not survive the field. Salaudeen (arXiv:2505.10573) and Bean (arXiv:2511.04703) are the nearest neighbours, positioned in Table 1 rather than argued with. Langer (M&M 35(1)), Buijsman (M&M 36(14)) and Barman (M&M 34(6)) are the venue-local pool whose work §4.5 and §6.2 rest on. Institutional addresses are the corresponding-author addresses on those papers and must be read off them rather than reconstructed. Springer's own position on the request is split: the M&M note says "the author should suggest up to six reviewers," while the general policy says authors "are welcome to suggest" and the journal "may not use the suggestions." Whether Editorial Manager enforces it is unknown until the form.

2026-08-09 — Ran `/submission-gate` for Minds and Machines. Quotation gate, terminology gate and legibility gate all PASS; package assembly not started.

Quotation gate found one real defect. The Kane p.64 quotation dropped an internal citation without an ellipsis: the source reads "only as strong as its weakest link (Crooks et al., 1996), and strong evidence". Restored, since APA 7 keeps citations embedded in quoted material and does not add them to the reference list. Page numbers verified independently, which the tool does not check: Kane p.64 is right (PDF p65, article pp. 1--73), Cronbach p.290 is right (article starts p.281), and the Magesh vendor quotation gained the page cite it lacked (p.234). That last one first looked like the edition trap the gate warns about, present in the JELS `.md` and absent from the JELS `.pdf`; the cause was soft hyphens in the PDF ("hallucination-­f ree"), not a difference between versions.

Terminology gate needed `planning/terms.md`, which did not exist. Written, with a per-term reason rather than a blanket exemption. Two terms needed real work rather than a ledger row: `profile` had eleven uses and no gloss, and the abstract named the projectibility audit without saying what it is. Both glossed; abstract now 228 words, inside the 150--250 limit.

Legibility gate: 0 MISSING on problem, debate, gap and contribution from three readers; 2/3 advance; problem statements convergent. Note the first run was contaminated and its result discarded: `coldread.py` strips `\citep` to empty brackets and `\ref` to a bare "Section", so its extract reads as an unproofed draft, and the reader rejected partly on that. Re-run from the typeset PDF, the same model advanced. **Tool defect worth fixing: `coldread.py` also mangles the title, rendering `\\Projectibility` as `\`.**

**The finding that matters is unanimous and the gate does not capture it.** All three readers, including both that advanced, doubted the contribution exceeds Kane restated. Sec. 2.3 answers that directly, with Kane's own chain-reasoning passage, but it sits on p.6, outside the desk-editor window. One reader's version is not answered anywhere: that the problem is "already covered by standard external-validity/generalizability checklists under other names". Checked: "generalizability" occurs once in the paper, in Sec. 6.1 citing Brennan on weighting, and "external validity" occurs zero times. Table 1 positions argument-based validity, nomological networks, estimand specification and causal transportability, but not G-theory. For a measurement-literate editor that is the first thing to come to mind, and the paper is silent on it.

2026-08-09 — Added generalizability theory as a positioned neighbour, closing the one cold-read objection the paper had no answer to anywhere. Before this, "generalizability" appeared once in 34 pages (Brennan on weighting in §6.1) and "external validity" zero times, while Table 1 positioned four rival frameworks and not G-theory. For a measurement-trained reader that is the first association, and a desk editor would have found nothing.

Three insertions. §1's neighbours list now names generalizability designs, which puts it inside the first two pages where all three cold readers stopped. Table 1 gains a row: universe of admissible observations and variance components; how far a score generalizes across the facets this design declared; and the residual question, whether two studies declare the same universe and if not which facet bridges them. §2.3 gains a paragraph before causal transportability.

The paragraph is deliberately generous rather than dismissive, and the source made that easy. Brennan works an example in which two investigators generalize to different universes from the same variance components (p. 14, verified against the PDF, printed page confirmed from the running head). So G-theory already names the difference this paper is about; what it does within one design, the audit asks across two, and neither separately conducted design contains the facet that would show whether they meet. Widening a G-study to span both is named as one legitimate form of the owed bridge. Claiming the framework is blind to the problem would have been both wrong and easy for a referee to refute.

34 pp, 0 undefined, quotation and terminology gates still pass.

2026-08-09 — Process finding: the submission gate ran after the arXiv v2 submission, not before. The gate describes itself as "the pre-flight gate before a manuscript leaves the workspace," and posting a preprint is the manuscript leaving the workspace, so the ordering was wrong. The consequence is concrete: the G-theory gap was present in v1 (posted 2026-07-28) and in v2 (submitted today, still processing), and the cold read that found it had never been run on this paper. Worth encoding as a trigger in the /arxiv-post skill: run /submission-gate before building the package, not after.

2026-08-09 — arXiv v2 unsubmitted and resubmitted the same day, still `submit/7929512`, to carry the generalizability-theory positioning. The window existed because it was a Sunday and arXiv announces on weekdays only, and because arXiv's documented behaviour is that "no additional versions are generated when edits are done before the submission is publicly announced" — so this remains v2 with no public trace of the round trip. Unsubmit rather than Update was the right control: arXiv's help names Unsubmit as the route that "will return it to incomplete status and allow you to modify your files", while Update appears to reach metadata only, and Delete would have destroyed the submission.

The ordering error that made this necessary is the finding worth keeping. The submission gate ran after the arXiv submission rather than before, and it describes itself as "the pre-flight gate before a manuscript leaves the workspace." Posting a preprint is the manuscript leaving the workspace. The gate's cold read had never been run on this paper, and it surfaced an objection the paper answered nowhere: that the problem is already covered by external-validity or generalizability checklists under other names. Had the gate run first, v1 would not have carried the gap either.

**To verify on announcement:** v1 is 2,852 KB and the resubmitted package is 2.6 MB, so the size on the abstract page distinguishes the new bundle from a resubmission of the old one. Also confirm the comments field and abstract were replaced rather than left at v1's text.

2026-08-09 — Built the blind source bundle Springer requires ("failing to submit a complete set of editable source files will result in your article not being considered for review"), and found that the blind PDF was not sufficient. `\anontrue` *skips* the non-blind branches at compile time; it does not remove them from the file. A reviewer opening `manuscript.tex` would have read the author block, the ORCID, the email, both affiliations, the companion repository URL, and both real self-citation keys, all sitting in `\else` branches. The house preamble additionally carried a "Brett Reynolds house style" comment and `pdfauthor={Brett Reynolds}`.

The bundle now resolves all five `\ifanon` branches at build time, keeping only the anonymous text, and redacts the two preamble lines. Verified: zero identity hits across `manuscript.tex`, `preamble.tex` and `manuscript.bbl`, and the redacted source rebuilds to a PDF matching `blind-manuscript.pdf` at 16,806 word tokens with two differing (one hyphenated word wrapping).

Worth generalising: an `\ifanon` switch makes a blind *PDF*, not a blind *submission*. Any venue that requires source needs the branches resolved, not skipped. This should go into the submission-gate skill's blinding checks, which currently name "self-citations and file names" and "local paths in supplementary files" but not the conditional-branch case.

2026-08-09 - Venue decision: Minds and Machines for "When Benchmark Inferences Do Not Compose: Projectibility in AI Evaluation". Owner: Brett, approved 2026-08-09. Record: `submission/venue-decision-2026-08-09.md`. Reason: the paper changes how M&M readers license a conclusion from benchmark evidence, and it argues with four M&M papers in four sections (Langer 35(1) and Buijsman 36(14) are load-bearing in §4.5, Barman 34(6) in §6.2, Bachmann 34(37) in §7.1). Risks accepted: the reviewer pool that fits the paper best is substantially the pool whose work it constrains; and the referee objection that the contribution may not exceed Kane restated, which §2.3 answers and §1 now signposts. Forecasts: P(reaches external review) 0.75, P(accepted) 0.30, first decision by 2026-10-15. Those are the agent's estimates adopted by Brett rather than formed independently, which weakens their calibration value and is recorded as such in the ledger.

2026-08-09 — Portal-fields record complete, section 11 empty, cleared to open Editorial Manager. Toronto leads the affiliation because Springer checks read-and-publish eligibility against the corresponding author's listed institution and U of T is paying the APC; Humber is second. Six suggested reviewers with addresses read off their own papers rather than recalled: Freiesleben (LMU), Salaudeen (MIT, not Stanford as first assumed), Bean (Oxford Internet Institute), Langer (Freiburg), Buijsman (TU Delft), Barman (Radboud, since moved to Ghent, so that address may bounce). One transcription hazard recorded: `pdftotext` ate the hyphen in Langer's domain at a line break, extracting "unifreiburg" where the address is `uni-freiburg.de`.

2026-08-09 — Submitted to *Minds and Machines* via Editorial Manager, status New Submission. Uploaded: the anonymized manuscript PDF and the separate non-anonymous title page. **The LaTeX source bundle was not uploaded**, and the guidelines say "failing to submit a complete set of editable source files will result in your article not being considered for review." The comments-to-office field discloses that an anonymized source bundle exists and can be supplied immediately or at revision, which converts a possible bounce into an email. `submission/blind-source.tar.gz` is built, leak-scanned and ready; if the office asks, upload `manuscript.tex` first as Manuscript, the preamble, `.bbl` and fonts as LaTeX Supporting Files, and re-type the PDF as supplementary, per Springer's LaTeX tips.

Also disclosed to the office: the arXiv preprint, no simultaneous submission, the blinding work, and that LLM use is declared in full on the title page because it went beyond copy editing.
